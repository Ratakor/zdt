...
pub fn main() !void {
    ...
    const now_berlin: Datetime = try Datetime.now(.{ .tz = &tz_berlin });
    ...
    defer my_tz.deinit();
    const now_local = try now_berlin.tzConvert(.{ .tz = &my_tz });
    ...
    defer tz_ny.deinit();
    const now_ny: Datetime = try now_local.tzConvert(.{ .tz = &tz_ny });
